...
public class Utils {
    ...
    private static Bundle decodeUrl(String s) {
        ...
        if (s != null) {
            ...
            for (String parameter : array) {
                ...
                try {
                    params.putString(URLDecoder.decode(v[0], JdConstant.DEFAULT_CHARSET),
                            URLDecoder.decode(v[1], JdConstant.DEFAULT_CHARSET));
    ...
    public static String openUrl(String url, String method, Bundle params, boolean multipart)
        ...
        String strBoundary = "c4db0637cb070be49a7356f7d1905ff2";
        String endLine = "\r\n";
        ...
        conn.setRequestProperty("User-Agent", System.getProperties().
                getProperty("http.agent") + JdConstant.USAGE_AGENT);
        if (!"GET".equals(method)) {
            ...
            if (params.containsKey("access_token")) {
                String decoded_token =
                        URLDecoder.decode(params.getString("access_token"), JdConstant.DEFAULT_CHARSET);
            ...
            conn.setRequestProperty("Accept-Encoding", JdConstant.ACCEPT_ENCODING);
            conn.connect();
            os = new BufferedOutputStream(conn.getOutputStream());

            if (multipart) {
                os.write(("--" + strBoundary + endLine).getBytes(JdConstant.DEFAULT_CHARSET));
                os.write((encodePostBody(params, strBoundary)).getBytes(JdConstant.DEFAULT_CHARSET));
                os.write((endLine + "--" + strBoundary + endLine)
                        .getBytes(JdConstant.DEFAULT_CHARSET));
            } else {
                os.write(buildQuery(params, JdConstant.DEFAULT_CHARSET).getBytes(
                        JdConstant.DEFAULT_CHARSET));
            }
        ...
        if (header != null && header.toLowerCase(Locale.CHINA).contains(JdConstant.ACCEPT_ENCODING)) {
            isGzip = true;
        }
        String response = "";
        try {
    ...
    public static String encodeUrl(Bundle parameters) {
        ...
        for (String key : parameters.keySet()) {
            if (first)
                ...
                sb.append("&");
            try {
                sb.append(URLEncoder.encode(key, JdConstant.DEFAULT_CHARSET))
                        .append("=")
    ...
    public static String buildQuery(Bundle param, String charset) {
        ...
        if (TextUtils.isEmpty(charset)) {
            charset = JdConstant.DEFAULT_CHARSET;
        }
    ...
    private static String getResponseCharset(String contentType) {
        String charset = JdConstant.DEFAULT_CHARSET;
